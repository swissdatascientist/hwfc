...
/* 
 * Straightforward implementation of Vector addition
 * 
 */
...
#else
        z[i] = z[i]+u[i]*v[i]+x[i]*y[i]+u[i]*y[i]+x[i]*v[i]+u[i]*x[i];
#endif
    }
}
...
int main(int argc, char **argv){
  ...
  if (argc==1) {printf("usage: FW <n>\n"); return -1;}
      ...
      FILE *fp;
      fp = fopen("/home/mfuhr/hwfc/01_exercise/01_assignment/peranalysis/performance_02.txt", "a");
      
      for(int k = nstart; k <= nend; k += nsteps){
          n = 1;
          for(int j = 0; j<k; j++) n = n*2;
